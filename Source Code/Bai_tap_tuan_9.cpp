...
void ReplaceElementB(int arrB[], int n) { // parameter : tham so
	for (int i = 0; i < n; i++) {
		if (arrB[i] < 0) {
			arrB[i] = 0;
		}
...
void SortAscendingB(int arrB[], int n) {
	for (int i = 0; i < n; i++) { // 5 4 3 2 1 // 4 5 3 2 1 //
		for (int j = i + 1; j < n; j++) { // 5 4
			if (arrB[i] > arrB[j]) {
				int temp = arrB[i]; // temp = arrB[i] => temp = 5
				arrB[i] = arrB[j]; // 5 = 4 // 4 4 3 2 1
				arrB[j] = temp; // 4 = 5 // 4 5 3 2 1 // 4 3 5 2 1 // 4 3 2 1 5 // 3 2 1 4 5 // 2 1 3 4 5 // 1 2 3 4 5
			}
		}
	}
}
...
double AverageOdd(int arr[], int n) { // y tuong : 1 2 3 4 5 // 1 2 2 3 5 
	int sum_odd = 0, count_odd = 0;
	for (int i = 0; i < n; i += 2) {
		if (arr[i] % 2 != 0) {
			sum_odd += arr[i]; // 1 = 1 + 3 // 4 // 4 = 4 + 5 = 9
			count_odd++; // 3 // sum_odd = 9 count_odd = 3
		}
	}
	double Average_odd = double(sum_odd) / count_odd; // 3
	return Average_odd;
}
...
int CountOccurrences(int arr[], int n, int b) { // 1 3 3 3 4 5 // 
...
int main() {
	...
	ReplaceElementB(arrB, n); // argument
	...
	std::cout << "\n7.\tCac phan tu cua mang B sap xep theo thu tu tang dan : ";
	for (int i = 0; i < n; i++) {
		std::cout << arrB[i] << " ";
	}
	std::cout << std::endl;
	...
	std::cout << "\n8.\tTrung binh cong cac so le o vi tri chan trong mang B : " << AverageOdd(arrB, n) << std::endl;

	// Nhap phan tu va tim phan tu dau tien = a
	int a; std::cout << "\n9.\tNhap phan tu a can tim : "; std::cin >> a;
	int vi_tri = FindFirstOccurrence(arrA, n, a); // 1 3 3 3 4 5 // 1
	if (vi_tri == -1) {
		std::cout << "9.\tKhong tim thay " << a << " trong mang A\n";
	}
	else {
		std::cout << "9.\tTim thay " << a << " tai vi tri " << vi_tri + 1 << " trong mang A\n";
	}

	// Nhap b va tim so lan xuat hien cua b trong mang A
	int b; std::cout << "\n10.\tNhap gia tri b : "; std::cin >> b;
	std::cout << "10.\tSo lan xuat hien cua gia tri b trong mang A la : " << CountOccurrences(arrA, n, b) << std::endl;

	// Gia tri dau tien cua mang A chu so dau tien la so chan
	std::cout << "\n11.\tGia tri dau tien trong mang A co chu so dau tien la so chan la : " << FindFirstEven(arrA, n) << std::endl;
